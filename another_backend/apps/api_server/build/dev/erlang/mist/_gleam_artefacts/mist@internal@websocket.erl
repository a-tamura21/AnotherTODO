...
-type valid_message(MQH) :: {socket_message, bitstring()} |
    socket_closed_message |
    {user_message, MQH}.

-type websocket_message(MQI) :: {valid, valid_message(MQI)} | invalid.
...
-type handler_message(MQJ) :: {internal, gramps@websocket:frame()} | {user, MQJ}.

-type websocket_state(MQK) :: {websocket_state,
        bitstring(),
        MQK,
...
?DOC(false).
-spec map_user_selector(gleam@option:option(gleam@erlang@process:selector(MRW))) -> gleam@option:option(gleam@erlang@process:selector(websocket_message(MRW))).
...
-spec apply_frames(
    ...
    fun((MRM, handler_message(MRN), websocket_connection()) -> mist@internal@next:next(MRM, MRN)),
    websocket_connection(),
    mist@internal@next:next(MRM, websocket_message(MRN)),
    fun((MRM) -> nil)
) -> mist@internal@next:next(MRM, websocket_message(MRN)).
...
-spec initialize_connection(
    fun((websocket_connection()) -> {MQW,
        gleam@option:option(gleam@erlang@process:selector(MQX))}),
    fun((MQW) -> nil),
    fun((MQW, handler_message(MQX), websocket_connection()) -> mist@internal@next:next(MQW, MQX)),
    glisten@socket:socket(),
    glisten@transport:transport(),
    list(binary())
) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(websocket_message(MQX)))} |
    {error, nil}.
