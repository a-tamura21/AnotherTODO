...
-opaque next(NFM, NFN) :: {continue,
        NFM,
        gleam@option:option(gleam@erlang@process:selector(NFN))} |
    normal_stop |
...
-opaque builder(NFO, NFP) :: {builder,
        integer(),
        fun((gleam@http@request:request(NFO)) -> gleam@http@response:response(NFP)),
...
-type websocket_message(NFQ) :: {text, binary()} |
    {binary, bitstring()} |
    closed |
    shutdown |
    {custom, NFQ}.
...
-spec continue(NFU) -> next(NFU, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\mist.gleam", 60).
-spec with_selector(next(NFY, NFZ), gleam@erlang@process:selector(NFZ)) -> next(NFY, NFZ).
...
-spec convert_next(next(NGN, NGO)) -> mist@internal@next:next(NGN, NGO).
convert_next(Next) ->
    case Next of
...
-spec new(
    fun((gleam@http@request:request(NHP)) -> gleam@http@response:response(NHR))
) -> builder(NHP, NHR).
new(Handler) ->
    {builder,
        4000,
...
-spec port(builder(NHV, NHW), integer()) -> builder(NHV, NHW).
port(Builder, Port) ->
    _record = Builder,
    {builder,
        Port,
...
-spec read_request_body(
    builder(bitstring(), NIB),
    integer(),
    gleam@http@response:response(NIB)
) -> builder(mist@internal@http:connection(), NIB).
...
-spec bind(builder(NIN, NIO), binary()) -> builder(NIN, NIO).
...
-spec with_ipv6(builder(NIT, NIU)) -> builder(NIT, NIU).
with_ipv6(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec with_tls(builder(NIZ, NJA), binary(), binary()) -> builder(NIZ, NJA).
...
-spec internal_to_public_ws_message(
    mist@internal@websocket:handler_message(NJP)
) -> {ok, websocket_message(NJP)} | {error, nil}.
...
-spec websocket(
    gleam@http@request:request(mist@internal@http:connection()),
    fun((NJV, websocket_message(NJW), mist@internal@websocket:websocket_connection()) -> next(NJV, NJW)),
    fun((mist@internal@websocket:websocket_connection()) -> {NJV,
        gleam@option:option(gleam@erlang@process:selector(NJW))}),
    fun((NJV) -> nil)
...
-spec server_sent_events(
    ...
    fun((gleam@erlang@process:subject(NKK)) -> {ok,
            gleam@otp@actor:initialised(NKM, NKK, any())} |
        {error, binary()}),
    fun((NKM, NKK, s_s_e_connection()) -> gleam@otp@actor:next(NKM, NKK))
) -> gleam@http@response:response(response_data()).
