...
-type message(FLS) :: {message, FLS} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-opaque next(FLT, FLU) :: {continue,
        FLT,
        gleam@option:option(gleam@erlang@process:selector(FLU))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type self(FLV, FLW) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        FLV,
        gleam@erlang@process:selector(message(FLW)),
        gleam@otp@system:debug_state(),
        fun((FLV, FLW) -> next(FLV, FLW))}.

-type started(FLX) :: {started, gleam@erlang@process:pid_(), FLX}.

-opaque initialised(FLY, FLZ, FMA) :: {initialised,
        FLY,
        gleam@option:option(gleam@erlang@process:selector(FLZ)),
        FMA}.

-opaque builder(FMB, FMC, FMD) :: {builder,
        fun((gleam@erlang@process:subject(FMC)) -> {ok,
                initialised(FMB, FMC, FMD)} |
            {error, binary()}),
        integer(),
        fun((FMB, FMC) -> next(FMB, FMC)),
        gleam@option:option(gleam@erlang@process:name(FMC))}.

-type start_error() :: init_timeout |
    {init_failed, binary()} |
    {init_exited, gleam@erlang@process:exit_reason()}.

-type start_init_message(FME) :: {ack, {ok, FME} | {error, binary()}} |
...
-spec continue(FMJ) -> next(FMJ, any()).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(FMV, FMW), gleam@erlang@process:selector(FMW)) -> next(FMV, FMW).
...
-spec initialised(FNC) -> initialised(FNC, any(), nil).
initialised(State) ->
...
-spec selecting(
    initialised(FNH, any(), FNJ),
    gleam@erlang@process:selector(FNN)
) -> initialised(FNH, FNN, FNJ).
...
-spec returning(initialised(FNS, FNT, any()), FNY) -> initialised(FNS, FNT, FNY).
...
-spec new(FOC) -> builder(FOC, FOD, gleam@erlang@process:subject(FOD)).
new(State) ->
...
-spec new_with_initialiser(
    integer(),
    fun((gleam@erlang@process:subject(FOI)) -> {ok, initialised(FOK, FOI, FOL)} |
        {error, binary()})
) -> builder(FOK, FOI, FOL).
...
-spec on_message(builder(FOU, FOV, FOW), fun((FOU, FOV) -> next(FOU, FOV))) -> builder(FOU, FOV, FOW).
...
-spec named(builder(FPF, FPG, FPH), gleam@erlang@process:name(FPG)) -> builder(FPF, FPG, FPH).
named(Builder, Name) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec select_system_messages(gleam@erlang@process:selector(message(FPU))) -> gleam@erlang@process:selector(message(FPU)).
...
-spec receive_message(self(any(), FPQ)) -> message(FPQ).
...
-spec initialise_actor(
    builder(any(), any(), FQN),
    gleam@erlang@process:pid_(),
    gleam@erlang@process:subject({ok, FQN} | {error, binary()})
...
-spec start(builder(any(), any(), FRA)) -> {ok, started(FRA)} |
    {error, start_error()}.
start(Builder) ->
...
-spec send(gleam@erlang@process:subject(FRH), FRH) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(FRJ),
    integer(),
    fun((gleam@erlang@process:subject(FRL)) -> FRJ)
) -> FRL.
