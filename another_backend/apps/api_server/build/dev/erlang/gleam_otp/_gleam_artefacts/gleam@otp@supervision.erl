...
-type child_specification(GEX) :: {child_specification,
        fun(() -> {ok, gleam@otp@actor:started(GEX)} |
...
-spec worker(
    fun(() -> {ok, gleam@otp@actor:started(GEY)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GEY).
worker(Start) ->
...
-spec supervisor(
    fun(() -> {ok, gleam@otp@actor:started(GFD)} |
        {error, gleam@otp@actor:start_error()})
) -> child_specification(GFD).
supervisor(Start) ->
...
-spec significant(child_specification(GFI), boolean()) -> child_specification(GFI).
...
-spec timeout(child_specification(GFL), integer()) -> child_specification(GFL).
timeout(Child, Ms) ->
...
-spec restart(child_specification(GFO), restart()) -> child_specification(GFO).
...
-spec map_data(child_specification(GFR), fun((GFR) -> GFT)) -> child_specification(GFT).
