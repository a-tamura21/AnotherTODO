...
-type message(IKS) :: {internal, internal_message()} | {user, IKS}.

-type loop_message(IKT) :: {packet, bitstring()} | {custom, IKT}.

-type loop_state(IKU, IKV) :: {loop_state,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        gleam@erlang@process:subject(message(IKV)),
        glisten@transport:transport(),
        IKU}.

-type connection(IKW) :: {connection,
        {ok, {glisten@socket@options:ip_address(), integer()}} | {error, nil},
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(message(IKW))}.

-type next(IKX, IKY) :: {continue,
        IKX,
        gleam@option:option(gleam@erlang@process:selector(IKY))} |
    normal_stop |
    {abnormal_stop, binary()}.

-type handler(IKZ, ILA) :: {handler,
        glisten@socket:socket(),
        fun((IKZ, loop_message(ILA), connection(ILA)) -> next(IKZ, loop_message(ILA))),
        fun((connection(ILA)) -> {IKZ,
            gleam@option:option(gleam@erlang@process:selector(ILA))}),
        gleam@option:option(fun((IKZ) -> nil)),
        glisten@transport:transport()}.

-file("src\\glisten\\internal\\handler.gleam", 65).
?DOC(false).
-spec continue(ILN) -> next(ILN, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\glisten\\internal\\handler.gleam", 69).
?DOC(false).
-spec with_selector(next(ILR, ILS), gleam@erlang@process:selector(ILS)) -> next(ILR, ILS).
...
?DOC(false).
-spec start(handler(any(), IMH)) -> {ok,
        gleam@otp@actor:started(gleam@erlang@process:subject(message(IMH)))} |
