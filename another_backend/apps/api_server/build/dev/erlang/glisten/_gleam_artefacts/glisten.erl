...
-type message(JFG) :: {packet, bitstring()} | {user, JFG}.
...
-type connection(JFH) :: {connection,
        glisten@socket:socket(),
        glisten@transport:transport(),
        gleam@erlang@process:subject(glisten@internal@handler:message(JFH))}.

-opaque next(JFI, JFJ) :: {continue,
        JFI,
        gleam@option:option(gleam@erlang@process:selector(JFJ))} |
    normal_stop |
    {abnormal_stop, binary()}.

-opaque builder(JFK, JFL) :: {builder,
        glisten@socket@options:interface(),
        fun((connection(JFL)) -> {JFK,
            gleam@option:option(gleam@erlang@process:selector(JFL))}),
        fun((JFK, message(JFL), connection(JFL)) -> next(JFK, message(JFL))),
        gleam@option:option(fun((JFK) -> nil)),
        integer(),
        boolean(),
        boolean(),
...
-spec continue(JGL) -> next(JGL, any()).
continue(State) ->
    {continue, State, none}.

-file("src\\glisten.gleam", 151).
-spec with_selector(next(JGP, JGQ), gleam@erlang@process:selector(JGQ)) -> next(JGP, JGQ).
...
?DOC(false).
-spec convert_next(next(JHE, JHF)) -> glisten@internal@handler:next(JHE, JHF).
convert_next(Next) ->
    case Next of
...
?DOC(false).
-spec map_selector(next(JHK, JHL), fun((JHL) -> JHO)) -> next(JHK, JHO).
...
-spec map_user_selector(gleam@erlang@process:selector(message(JHR))) -> gleam@erlang@process:selector(glisten@internal@handler:loop_message(JHR)).
...
-spec convert_loop(
    fun((JHW, message(JHX), connection(JHX)) -> next(JHW, message(JHX)))
) -> fun((JHW, glisten@internal@handler:loop_message(JHX), glisten@internal@handler:connection(JHX)) -> glisten@internal@handler:next(JHW, glisten@internal@handler:loop_message(JHX))).
convert_loop(Loop) ->
    fun(Data, Msg, Conn) ->
        Conn@1 = {connection,
            erlang:element(3, Conn),
...
-spec convert_on_init(
    fun((connection(JIC)) -> {JIE,
        gleam@option:option(gleam@erlang@process:selector(JIC))})
) -> fun((glisten@internal@handler:connection(JIC)) -> {JIE,
    gleam@option:option(gleam@erlang@process:selector(JIC))}).
...
-spec new(
    fun((connection(JIK)) -> {JIM,
        gleam@option:option(gleam@erlang@process:selector(JIK))}),
    fun((JIM, message(JIK), connection(JIK)) -> next(JIM, message(JIK)))
) -> builder(JIM, JIK).
new(On_init, Loop) ->
...
-spec with_close(builder(JIT, JIU), fun((JIT) -> nil)) -> builder(JIT, JIU).
...
-spec with_pool_size(builder(JIZ, JJA), integer()) -> builder(JIZ, JJA).
...
?DOC(false).
-spec with_http2(builder(JJF, JJG)) -> builder(JJF, JJG).
with_http2(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec with_ipv6(builder(JJR, JJS)) -> builder(JJR, JJS).
with_ipv6(Builder) ->
    _record = Builder,
    {builder,
        erlang:element(2, _record),
...
-spec with_tls(builder(JJX, JJY), binary(), binary()) -> builder(JJX, JJY).
...
-spec bind(builder(JJL, JJM), binary()) -> builder(JJL, JJM).
