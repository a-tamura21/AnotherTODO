...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(FIW)) -> {ok, FIW} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(FIQ)) -> {ok, FIQ} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(FIM)) -> {ok, FIM} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(FJC), fun((FJC) -> json())) -> json().
...
-spec array(list(FJG), fun((FJG) -> json())) -> json().
