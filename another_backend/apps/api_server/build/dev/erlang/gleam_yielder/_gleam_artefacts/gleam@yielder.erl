...
-type action(GLH) :: stop | {continue, GLH, fun(() -> action(GLH))}.

-opaque yielder(GLI) :: {yielder, fun(() -> action(GLI))}.

-type step(GLJ, GLK) :: {next, GLJ, GLK} | done.

-type chunk(GLL, GLM) :: {another_by,
        list(GLL),
        GLM,
        GLL,
        fun(() -> action(GLL))} |
...
-type sized_chunk(GLN) :: {another, list(GLN), fun(() -> action(GLN))} |
    {last, list(GLN)} |
    no_more.
...
-spec unfold_loop(GLV, fun((GLV) -> step(GLW, GLV))) -> fun(() -> action(GLW)).
...
-spec unfold(GLQ, fun((GLQ) -> step(GLR, GLQ))) -> yielder(GLR).
unfold(Initial, F) ->
    _pipe = Initial,
...
-spec repeatedly(fun(() -> GMA)) -> yielder(GMA).
repeatedly(F) ->
...
-spec from_list(list(GME)) -> yielder(GME).
from_list(List) ->
...
-spec transform(yielder(GMO), GMQ, fun((GMQ, GMO) -> step(GMR, GMQ))) -> yielder(GMR).
transform(Yielder, Initial, F) ->
    _pipe = transform_loop(erlang:element(2, Yielder), Initial, F),
    {yielder, _pipe}.

-file("src\\gleam\\yielder.gleam", 204).
-spec fold_loop(fun(() -> action(GMY)), fun((GNA, GMY) -> GNA), GNA) -> GNA.
...
-spec fold(yielder(GMV), GMX, fun((GMX, GMV) -> GMX)) -> GMX.
...
-spec to_list(yielder(GND)) -> list(GND).
to_list(Yielder) ->
    _pipe = Yielder,
...
-spec step(yielder(GNG)) -> step(GNG, yielder(GNG)).
step(Yielder) ->
...
-spec take_loop(fun(() -> action(GNO)), integer()) -> fun(() -> action(GNO)).
...
-spec take(yielder(GNL), integer()) -> yielder(GNL).
...
-spec drop_loop(fun(() -> action(GNU)), integer()) -> action(GNU).
...
-spec map(yielder(GNX), fun((GNX) -> GNZ)) -> yielder(GNZ).
map(Yielder, F) ->
    _pipe = erlang:element(2, Yielder),
...
-spec map2(yielder(GOF), yielder(GOH), fun((GOF, GOH) -> GOJ)) -> yielder(GOJ).
...
-spec append_loop(fun(() -> action(GOV)), fun(() -> action(GOV))) -> action(GOV).
...
-spec append(yielder(GOR), yielder(GOR)) -> yielder(GOR).
append(First, Second) ->
    _pipe = fun() ->
...
-spec flatten_loop(fun(() -> action(yielder(GPD)))) -> action(GPD).
...
-spec flatten(yielder(yielder(GOZ))) -> yielder(GOZ).
flatten(Yielder) ->
...
-spec concat(list(yielder(GPH))) -> yielder(GPH).
concat(Yielders) ->
...
-spec flat_map(yielder(GPL), fun((GPL) -> yielder(GPN))) -> yielder(GPN).
flat_map(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    flatten(_pipe@1).
...
-spec filter_loop(fun(() -> action(GPT)), fun((GPT) -> boolean())) -> action(GPT).
...
-spec filter(yielder(GPQ), fun((GPQ) -> boolean())) -> yielder(GPQ).
...
-spec filter_map(yielder(GPW), fun((GPW) -> {ok, GPY} | {error, any()})) -> yielder(GPY).
...
-spec cycle(yielder(GQK)) -> yielder(GQK).
cycle(Yielder) ->
    _pipe = repeat(Yielder),
    flatten(_pipe).

-file("src\\gleam\\yielder.gleam", 709).
-spec find_loop(fun(() -> action(GQS)), fun((GQS) -> boolean())) -> {ok, GQS} |
    {error, nil}.
...
-spec find(yielder(GQO), fun((GQO) -> boolean())) -> {ok, GQO} | {error, nil}.
...
-spec find_map(yielder(GQW), fun((GQW) -> {ok, GQY} | {error, any()})) -> {ok,
        GQY} |
    {error, nil}.
...
-spec index_loop(fun(() -> action(GRP)), integer()) -> fun(() -> action({GRP,
    integer()})).
...
-spec index(yielder(GRM)) -> yielder({GRM, integer()}).
index(Yielder) ->
    _pipe = erlang:element(2, Yielder),
...
-spec iterate(GRS, fun((GRS) -> GRS)) -> yielder(GRS).
iterate(Initial, F) ->
    unfold(Initial, fun(Element) -> {next, Element, F(Element)} end).

-file("src\\gleam\\yielder.gleam", 832).
-spec take_while_loop(fun(() -> action(GRX)), fun((GRX) -> boolean())) -> fun(() -> action(GRX)).
...
-spec take_while(yielder(GRU), fun((GRU) -> boolean())) -> yielder(GRU).
...
-spec drop_while_loop(fun(() -> action(GSD)), fun((GSD) -> boolean())) -> action(GSD).
...
-spec drop_while(yielder(GSA), fun((GSA) -> boolean())) -> yielder(GSA).
...
-spec scan_loop(fun(() -> action(GSK)), fun((GSM, GSK) -> GSM), GSM) -> fun(() -> action(GSM)).
...
-spec scan(yielder(GSG), GSI, fun((GSI, GSG) -> GSI)) -> yielder(GSI).
...
-spec zip_loop(fun(() -> action(GST)), fun(() -> action(GSV))) -> fun(() -> action({GST,
    GSV})).
zip_loop(Left, Right) ->
    fun() -> case Left() of
            stop ->
...
-spec zip(yielder(GSO), yielder(GSQ)) -> yielder({GSO, GSQ}).
zip(Left, Right) ->
    _pipe = zip_loop(erlang:element(2, Left), erlang:element(2, Right)),
    {yielder, _pipe}.

-file("src\\gleam\\yielder.gleam", 1000).
-spec next_chunk(fun(() -> action(GTI)), fun((GTI) -> GTK), GTK, list(GTI)) -> chunk(GTI, GTK).
...
-spec chunk_loop(fun(() -> action(GTD)), fun((GTD) -> GTF), GTF, GTD) -> action(list(GTD)).
...
-spec chunk(yielder(GSY), fun((GSY) -> any())) -> yielder(list(GSY)).
chunk(Yielder, F) ->
...
-spec next_sized_chunk(fun(() -> action(GTW)), integer(), list(GTW)) -> sized_chunk(GTW).
...
-spec sized_chunk_loop(fun(() -> action(GTS)), integer()) -> fun(() -> action(list(GTS))).
...
-spec sized_chunk(yielder(GTO), integer()) -> yielder(list(GTO)).
...
-spec intersperse_loop(fun(() -> action(GUD)), GUD) -> action(GUD).
...
-spec intersperse(yielder(GUA), GUA) -> yielder(GUA).
...
-spec any_loop(fun(() -> action(GUI)), fun((GUI) -> boolean())) -> boolean().
...
-spec any(yielder(GUG), fun((GUG) -> boolean())) -> boolean().
any(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    any_loop(_pipe, Predicate).

-file("src\\gleam\\yielder.gleam", 1228).
-spec all_loop(fun(() -> action(GUM)), fun((GUM) -> boolean())) -> boolean().
...
-spec all(yielder(GUK), fun((GUK) -> boolean())) -> boolean().
all(Yielder, Predicate) ->
    _pipe = erlang:element(2, Yielder),
    all_loop(_pipe, Predicate).

-file("src\\gleam\\yielder.gleam", 1273).
-spec update_group_with(GVC) -> fun((gleam@option:option(list(GVC))) -> list(GVC)).
...
-spec group_updater(fun((GUU) -> GUV)) -> fun((gleam@dict:dict(GUV, list(GUU)), GUU) -> gleam@dict:dict(GUV, list(GUU))).
group_updater(F) ->
...
-spec group(yielder(GUO), fun((GUO) -> GUQ)) -> gleam@dict:dict(GUQ, list(GUO)).
group(Yielder, Key) ->
    _pipe = Yielder,
...
-spec reduce(yielder(GVG), fun((GVG, GVG) -> GVG)) -> {ok, GVG} | {error, nil}.
reduce(Yielder, F) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec last(yielder(GVK)) -> {ok, GVK} | {error, nil}.
last(Yielder) ->
    _pipe = Yielder,
...
-spec single(GVS) -> yielder(GVS).
single(Elem) ->
    once(fun() -> Elem end).

-file("src\\gleam\\yielder.gleam", 1402).
-spec interleave_loop(fun(() -> action(GVY)), fun(() -> action(GVY))) -> action(GVY).
...
-spec interleave(yielder(GVU), yielder(GVU)) -> yielder(GVU).
...
-spec fold_until_loop(
    fun(() -> action(GWG)),
    fun((GWI, GWG) -> gleam@list:continue_or_stop(GWI)),
    GWI
) -> GWI.
...
-spec fold_until(
    yielder(GWC),
    GWE,
    fun((GWE, GWC) -> gleam@list:continue_or_stop(GWE))
) -> GWE.
...
-spec try_fold(yielder(GWK), GWM, fun((GWM, GWK) -> {ok, GWM} | {error, GWN})) -> {ok,
        GWM} |
    {error, GWN}.
...
-spec first(yielder(GXA)) -> {ok, GXA} | {error, nil}.
first(Yielder) ->
    case (erlang:element(2, Yielder))() of
        stop ->
            {error, nil};
...
-spec at(yielder(GXE), integer()) -> {ok, GXE} | {error, nil}.
at(Yielder, Index) ->
    _pipe = Yielder,
...
-spec each(yielder(GXM), fun((GXM) -> any())) -> nil.
each(Yielder, F) ->
    _pipe = Yielder,
    _pipe@1 = map(_pipe, F),
    run(_pipe@1).
...
-spec yield(GXP, fun(() -> yielder(GXP))) -> yielder(GXP).
yield(Element, Next) ->
    {yielder,
        fun() ->
...
-spec prepend(yielder(GXS), GXS) -> yielder(GXS).
