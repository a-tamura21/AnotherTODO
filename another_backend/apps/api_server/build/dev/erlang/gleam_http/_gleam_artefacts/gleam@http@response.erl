...
-type response(FBV) :: {response, integer(), list({binary(), binary()}), FBV}.
...
-spec set_header(response(FCK), binary(), binary()) -> response(FCK).
...
-spec prepend_header(response(FCN), binary(), binary()) -> response(FCN).
...
-spec set_body(response(any()), FCS) -> response(FCS).
...
-spec try_map(response(FBW), fun((FBW) -> {ok, FBY} | {error, FBZ})) -> {ok,
        response(FBY)} |
    {error, FBZ}.
...
-spec map(response(FCU), fun((FCU) -> FCW)) -> response(FCW).
...
-spec expire_cookie(response(FDE), binary(), gleam@http@cookie:attributes()) -> response(FDE).
