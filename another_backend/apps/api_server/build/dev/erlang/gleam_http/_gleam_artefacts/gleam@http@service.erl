...
-spec map_response_body(
    fun((FGI) -> gleam@http@response:response(FFU)),
    fun((FFU) -> FFV)
) -> fun((FGI) -> gleam@http@response:response(FFV)).
...
-spec prepend_response_header(
    fun((FGM) -> gleam@http@response:response(FGO)),
    binary(),
    binary()
) -> fun((FGM) -> gleam@http@response:response(FGO)).
...
-spec ensure_post(gleam@http@request:request(FFZ)) -> {ok,
        gleam@http@request:request(FFZ)} |
    {error, nil}.
ensure_post(Req) ->
...
-spec method_override(fun((gleam@http@request:request(FHV)) -> FID)) -> fun((gleam@http@request:request(FHV)) -> FID).
