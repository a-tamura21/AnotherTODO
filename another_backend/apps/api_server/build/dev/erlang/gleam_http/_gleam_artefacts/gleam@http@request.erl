...
-spec set_header(request(ESE), binary(), binary()) -> request(ESE).
...
-spec prepend_header(request(ESH), binary(), binary()) -> request(ESH).
...
-spec set_body(request(any()), ESM) -> request(ESM).
set_body(Req, Body) ->
...
-spec map(request(ESO), fun((ESO) -> ESQ)) -> request(ESQ).
...
-spec set_query(request(ETA), list({binary(), binary()})) -> request(ETA).
set_query(Req, Query) ->
    Pair = fun(T) ->
...
-spec set_method(request(ETE), gleam@http:method()) -> request(ETE).
...
-spec remove_cookie(request(EUC), binary()) -> request(EUC).
