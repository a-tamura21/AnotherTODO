...
-spec be_ok({ok, HTV} | {error, any()}) -> HTV.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HUA}) -> HUA.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HUD)) -> HUD.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
